...
class IceServiceCenter(models.Model):
    ...
    master_server = models.ForeignKey(
            Server, on_delete=models.PROTECT, verbose_name='注册中心(master)',
            related_name='master_server_set'
        )
    master_port = models.PositiveIntegerField(verbose_name='端口(master)')
    slave_server = models.ForeignKey(
            Server, on_delete=models.SET_NULL, verbose_name='注册中心(slave)',
            related_name='slave_server_set', blank=True, null=True
        )
    ...
    def get_all_registry_names(self):
        ...
        try:
            ...
            return admin.get_all_registry_names()
        finally:
            admin.destroy()
    ...
    def get_all_node_info(self):
        ...
        try:
            ...
            for node in admin.get_all_node_names():
                ...
                except (TypeError, icegridadmin.NodeUnreachableException):
                    continue
...
class IceService(models.Model):
    ...
    def to_task(self):
        ...
        return {
            ...
            'conf_md5': conf.md5 if conf else '',
        }
    ...
    def push_xml_to_registry(self):
        """同步xml文件到主从两个注册中心的config目录"""
        master = self.center.master_server  # 主注册
        slave = self.center.slave_server  # 从注册
        task = {
            'type': 'ice_sync_xml',
            'name': self.name,
            'priority': 1,
            'xml_url': urlparse.urljoin(settings.MAIN_URL,  self.xml.url),
            'to_dir': '/opt/ice_project/config',
            'xml_md5': file_md5(self.xml.path),
        }
        node = NodeProxy(master.ip, settings.NODE_PORT)
        node.add_task(task)
        node = NodeProxy(slave.ip, settings.NODE_PORT)
        node.add_task(task)
            ...
            if admin.application_exist(self.name):
                ...
                for server in servers:
                    try:
                        ...
                        logger.exception('ice server stop error - server-node: %s' % server)
                    try:
                        admin.start_server(server)
                    except Exception:
                        logger.exception('ice node start error - node: %s' % server)
            else:
...
class TomcatCluster(models.Model):
    ...
    class Meta:
        ...
        permissions = {
            ...
            ('push_static_pkg', "push static html package"),
        }
...
class TomcatServer(models.Model):
    ...
    static_dir = models.CharField(max_length=64, verbose_name='静态文件目录')
    note = models.TextField(blank=True, default='', verbose_name='描述')
    ...
    def kill(self):
        node = NodeProxy(self.host.ip, settings.NODE_PORT)
        try:
            return node.kill_tomcat(self.cmd)
...
class TomcatApp(models.Model):
    ...
    def get_active_static(self):
        return self.tomcatappstatic_set.filter(active=True).first()
    ...
    def push_static(self, static_pkg, tomcat_app_node, priority=1):
        task = {
            'type': 'tomcat_static',
            'host': tomcat_app_node.server.host.ip,
            'app_name': self.name,
            'identifier': self.hex_identifier,
            'priority': priority,
            'html_dir': tomcat_app_node.server.static_dir,
            'html_url': static_pkg.get_download_url(),
            'html_md5': static_pkg.md5,
        }
        node = NodeProxy(tomcat_app_node.server.host.ip, settings.NODE_PORT)
        return node.add_task(task)
...
class TomcatAppStatic(models.Model):
    package = models.FileField(upload_to='tomcat_apps/web_html/', verbose_name='静态文件(zip包)')
    note = models.TextField(verbose_name='描述', blank=True, default='')
    tomcat_app = models.ForeignKey(TomcatApp)
    active = models.BooleanField(default=False, verbose_name='激活')
    ...
    class Meta:
        verbose_name = 'Tomcat应用静态文件'
        verbose_name_plural = 'Tomcat应用静态文件'

    def __unicode__(self):
        return os.path.basename(self.package.path)

    def get_download_url(self):
        return urlparse.urljoin(settings.MAIN_URL, self.package.url)

    def get_active_url(self):
        return reverse('active_static', args=[self.id])

    def get_update_url(self):
        return reverse('tomcat_app_static_update', kwargs={'pk': self.pk})

    def get_delete_url(self):
        return reverse('tomcat_app_static_delete', kwargs={'pk': self.pk})


@receiver(models.signals.post_delete, sender=TomcatAppStatic)
def auto_delete_static_on_delete(sender, instance, **kwargs):
    if instance.package:
        if os.path.isfile(instance.package.path):
            os.remove(instance.package.path)


@receiver(models.signals.pre_save, sender=TomcatAppStatic)
def auto_delete_static_on_change(sender, instance, **kwargs):
    if instance.pk is None:
        return False
    try:
        old_static = TomcatAppStatic.objects.get(pk=instance.pk)
    except TomcatAppStatic.DoesNotExist:
        return False

    new_pkg = instance.package
    if old_static.package != new_pkg:
        if os.path.isfile(old_static.package.path):
            os.remove(old_static.package.path)
...
class TomcatAppSql(models.Model):
    ...
    tomcat_app = models.ForeignKey(TomcatApp)
    # 执行成功为空, 如果出错会保存错误信息
    stdout = models.TextField(blank=True, default='', verbose_name='执行日志')
    ...
    def get_download_url(self):
        return urlparse.urljoin(settings.MAIN_URL, self.sql.url)

    def get_absolute_url(self):
        return reverse('tomcat_app_sql_detail', kwargs={'pk': self.pk})
