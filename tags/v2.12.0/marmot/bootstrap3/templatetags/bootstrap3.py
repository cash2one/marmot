...
@register.simple_tag
def bootstrap_span_state(state, content_type=1):
    """
    Render an span

    **Tag name**::

        bootstrap_span_state
    ...
    **Example**::
    ...
    """
    if content_type == 1:
        content = 'Active' if state else 'Inactive'
    elif content_type == 2:
        content = '在线' if state else '离线'
    else:
        raise ValueError
    return render_span(content, span_type='success' if state else 'danger')


@register.simple_tag
def bootstrap_span_custom(state, text):
    """
    Render an span

    **Tag name**::

        bootstrap_span_custom
    ...
    **Usage**::
        ...
        {% bootstrap_span_custom state "True/False" %}

    **Example**::

        {% bootstrap_span_custom state "Active/Inactive" %}

    """
    text_true, text_false = text.split('/')
    content = text_true if state else text_false
    return render_span(content, span_type='success' if state else 'danger')
