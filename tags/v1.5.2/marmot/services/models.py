...
class TomcatApp(models.Model):
    name = models.CharField(max_length=48, verbose_name='名称')
    war_dir = models.ForeignKey(TomcatServerWarDir, verbose_name='war包目录')
    db_name = models.CharField(max_length=48, verbose_name='数据库名', blank=True, null=True)
    db_ip = models.GenericIPAddressField(verbose_name='数据库地址', blank=True, null=True)
    db_port = models.PositiveIntegerField(verbose_name='数据库端口', blank=True, null=True)
    db_user = models.CharField(max_length=48, verbose_name='数据库账号', blank=True, null=True)
    db_pwd = models.CharField(max_length=48, verbose_name='数据库密码', blank=True, null=True)
    bak_flag = models.BooleanField(default=False, verbose_name='正在备份数据库')
    note = models.TextField(blank=True, null=True, verbose_name='描述')
    tomcat_server = models.ForeignKey(TomcatServer)
    ...
    @property
    def db_is_enabled(self):
        return all([self.db_name, self.db_ip, self.db_port, self.db_user, self.db_pwd])

    def push_war(self, war_path, priority=1):
        task = {
            'type': 'tomcat',
            'host': self.tomcat_server.host.ip,
            'app_name': self.name,
            'identifier': self.hex_identifier,
            'priority': priority,
            'war_dir': self.war_dir.war_dir,
            'war_url': urlparse.urljoin(settings.MAIN_URL, war_path),
        }
        node = connect_node(self.tomcat_server.host.ip)
...
class TomcatAppWar(models.Model):
    ...
    note = models.TextField(verbose_name='描述', blank=True, null=True)
    ...
    def push_to_tomcat(self):
        return self.tomcat_app.push_war(self.package.url)
