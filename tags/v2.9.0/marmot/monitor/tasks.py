...
from .models import (
    RedisClusterMonitor, ESMonitor, Neo4jMonitor, HBaseClusterMonitor, IceGridMonitor
)
...
def es_monitor():
    ...
    for es in esm_list:
        ...
        if status == 'green':
            ...
            send_alarm_to_group(
                '警报: "ES集群: %s" - %s' % (es.name, es.addr), 'status: yellow',
                json.dumps(info, indent=2)
            )
        elif status == 'red':
            send_alarm_to_group(
                '警报: "ES集群: %s" - %s' % (es.name, es.addr), 'status: red',
                json.dumps(info, indent=2)
            )
        else:
