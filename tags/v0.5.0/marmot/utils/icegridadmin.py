...
class IceGridAdmin(object):
    ...
    def xml_to_app_descriptor(self, file_):
        """parse file.xml to application descriptor"""
    ...
    def get_server_state(self, id_):
        return self.admin_proxy.getServerState(id_)
    ...
    def get_application_nodes(self, name):
        ...
        if descriptor:
            for node in descriptor.nodes.values():
                node_id = node.serverInstances[0].parameterValues['id']
                nodes[node_id] = {'state': self.get_server_state(node_id), 'pid': self.get_server_pid(node_id)}
        return nodes
