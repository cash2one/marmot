...
"""
此版本主要是要执行HBase的shell来获取HBase的status信息, 用于监控.
只在有HBase的机器上安装此版本即可.
此版本依赖pexpect包。
HBase shell启动慢会阻塞Rpc的主线程, 这里对XmlRpcServer加入了多线程支持
"""
...
def task_factory(task_info):
    ...
    if type_ == 'tomcat_static':
        return TaskTomcatStatic(
            task_info['host'], task_info['app_name'], task_info['identifier'],
            task_info['priority'], task_info['html_url'], task_info['html_md5'], task_info['html_dir']
        )
    if type_ == 'script':
...
class XmlRpcServer(ThreadXMLRPCServer):
    allow_client_hosts = (MARMOT_HOST, 'localhost', '127.0.0.1', '10.20.0.202', '172.16.20.48')
...
class MarmotAgent(object):
    ...
    def create_from_cli(cls):
        ...
        try:
            conf = http_get_json(CONF_URL, {'hostname': socket.gethostname()})
        except IOError:
