...
class State(models.Model):
    ...
    # time, estimation_unit stores the unit of time estimation_value
    # TODO Maybe models.DurationField better
    estimation_value = models.IntegerField('过期时间长度值', default=0)
    estimation_unit = models.IntegerField('过期时间长度单位', default=DAY, choices=DURATIONS)
    ...
    def deadline(self):
        """
        :return datetime - 此状态的截止时间; None - 没有截止时间
        """
...
class WorkflowActivity(models.Model):
    """
    具体某个工作流的工单实例, 每开启一个工作流, 就生成一个WorkflowActivity的实例
    """
    ...
    def current_state(self):
        """
        返回该WorkflowActivity对象的最新的一个WorkflowHistory实例
        """
    ...
    def current_transitions(self):
        ...
        if current_state:
            return current_state.state.transitions_from.all()
        else:
            return None
    ...
    def start(self, user):
        ...
        if start_state_result.count() != 1:
            raise UnableToStartWorkflow('There are multiple start-state')

        transitions = self.workflow.transitions.filter(from_state=start_state_result)
...
class WorkflowHistory(models.Model):
    ...
    deadline = models.DateTimeField(
            ...
            help_text='The deadline for staying in this state'
        )
