...
class Server(models.Model):
    ...
    hostname = models.CharField(max_length=24, unique=True, verbose_name='主机名')
    type = models.IntegerField(choices=SERVER_TYPE, default=2, verbose_name='类型')
    master_host = models.ForeignKey(
            'Server', limit_choices_to={'type': 1}, verbose_name='宿主机',
    ...
    @property
    def is_alive(self):
        node = NodeProxy(self.ip, settings.NODE_PORT, timeout=5.0)
        try:
            node.is_alive()
            return True
    ...
    def get_runtime_data(self):
        node = NodeProxy(self.ip, NODE_PORT)
        try:
            return node.get_runtime_data()
        except (IOError, xmlrpclib.Fault, httplib.BadStatusLine):
            return None
    ...
    def get_connections(self, family='', type='', state=''):
        node = NodeProxy(self.ip, NODE_PORT)
        try:
            return node.get_connections(family, type, state)
        except (IOError, xmlrpclib.Fault, httplib.BadStatusLine):
            return None
    ...
    def get_network_interfaces(self):
        node = NodeProxy(self.ip, NODE_PORT)
        try:
            return node.get_network_interfaces()
        except (IOError, xmlrpclib.Fault, httplib.BadStatusLine):
            return None
