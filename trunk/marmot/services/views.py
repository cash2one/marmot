...
from .models import (
    IceServiceCenter, IceServiceConfig, IceServiceJar,
    IceService, TomcatGroup, TomcatCluster, TomcatServer,
    TomcatServerWarDir, TomcatApp, TomcatAppNode, TomcatAppSql,
    TomcatAppWar, TomcatAppStatic, TomcatAppDB
)
from .forms import (
    IceServiceCenterForm, IceServiceJarForm, IceServiceConfigForm,
    IceServiceForm, IceServiceUpdateForm, TomcatGroupForm,
    TomcatClusterForm, TomcatServerForm, TomcatServerWarDirForm,
    TomcatAppForm, TomcatAppUpdateForm, TomcatAppWarForm,
    TomcatAppDBForm, TomcatAppSqlForm, TomcatAppStaticForm
)
...
class IceServiceJarUpdate(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.object = self.get_object()
        if request.user.profile.role.alias == 'developer' \
                and request.user.profile.privilege < 3 \
                and request.user not in self.object.ice_service.users.all():
...
def active_jar_package(request, pk):
    ...
    jar.ice_service.iceservicejar_set.filter(active=True).update(active=False)
    jar.active = True
    jar.save()
...
def active_config(request, pk):
    ...
    conf.ice_service.iceserviceconfig_set.filter(active=True).update(active=False)
    conf.active = True
    conf.save()
    return JsonResponse({'msg': 0})
...
def push_pkg_to_node(request, service_id):
    ...
    try:
        ...
        logger.exception('Push task error')
        return JsonResponse({'msg': unicode(e)})
    return JsonResponse({'msg': 0})
...
def start_ice_service(request, service_id):
    ...
    try:
        ...
        try:
            service.push_xml_to_registry()
        except Exception:
            logger.exception('Sync xml to registry error')
    return JsonResponse({'msg': 0})
...
class TomcatAppSqlDetail(LoginRequiredMixin, DetailView):
    model = TomcatAppSql
    context_object_name = 'tomcat_app_sql'
    template_name = 'services/tomcat_app_sql_detail.html'

    def get_context_data(self, **kwargs):
        context = super(TomcatAppSqlDetail, self).get_context_data(**kwargs)
        return context
...
def execute_sql(request, pk):
    ...
    if sql.state == 3:
        return JsonResponse({'msg': '此sql已经执行过了'})
    if sql.state == 2:
