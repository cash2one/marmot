...
from .models import (
    ...
    TomcatApp, TomcatAppSql, TomcatAppWar, TomcatAppStatic, TomcatAppDB
)
...
class TomcatServerForm(ModelForm):
    ...
    def clean_static_dir(self):
        node = NodeProxy(self.cleaned_data['host'].ip, settings.NODE_PORT)
        try:
            if not node.path_exists(self.cleaned_data['static_dir']):
                raise forms.ValidationError('该目录不存在!')
        except (IOError, xmlrpclib.Fault, httplib.BadStatusLine):
            raise forms.ValidationError('连接不上服务器, 无法验证启动命令的正确性！')
        return self.cleaned_data['static_dir']
...
class TomcatAppStaticForm(ModelForm):
    class Meta:
    ...
    def save(self, commit=True):
        instance = super(TomcatAppStaticForm, self).save(commit=commit)
        if instance.package:
