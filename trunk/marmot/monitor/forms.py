...
from .models import (
    RedisClusterMonitor, RedisNode, ESMonitor,
    HBaseClusterMonitor, Neo4jMonitor, ActiveMqMonitor,
    SpringCloudMonitor, KafkaMonitor, ZooKeeperMonitor
)
...
class ActiveMqMonitorForm(ModelForm):
    class Meta:
        model = ActiveMqMonitor
        fields = ['name', 'addr', 'level', 'active', 'note', 'created_by']
        widgets = {
            'active': CheckboxInput(),
            'created_by': HiddenInput(),
        }
...
class SpringCloudMonitorForm(ModelForm):
    class Meta:
        model = SpringCloudMonitor
        fields = ['name', 'addr', 'port', 'active', 'note', 'created_by']
        widgets = {
            'active': CheckboxInput(),
            'created_by': HiddenInput(),
        }
...
class KafkaMonitorForm(ModelForm):
    class Meta:
        model = KafkaMonitor
        fields = ['name', 'addr', 'ids', 'replicas', 'isr', 'active', 'note', 'created_by']
        widgets = {
            'active': CheckboxInput(),
            'created_by': HiddenInput(),
        }
...
class ZooKeeperMonitorForm(ModelForm):
    class Meta:
        model = ZooKeeperMonitor
        fields = ['name', 'addr', 'active', 'note', 'created_by']
        widgets = {
            'active': CheckboxInput(),
            'created_by': HiddenInput(),
        }
