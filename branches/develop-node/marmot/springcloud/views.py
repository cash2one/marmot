...
@login_required
def springcloud_app_node_status(request):
    """判断springcloud应用指定节点的状态"""
    # TODO 暂时没有启用
    ...
    return JsonResponse({'msg': 0, 'status': sca.is_alive(sc_node)})
...
def springcloud_backup_create(request):
    ...
    try:
        ...
        logger.error('SpringCloudBackup Error - Server: %s DoesNotExist' % appname)
        return JsonResponse({'msg': 1})

    try:
        app = SpringCloudApp.objects.get(name=appname, cluster__springcloudnode__server=server)
...
def springcloud_backup_options(request):
    ...
    node = request.GET.get('node')
    try:
        scb_list = SpringCloudBackup.objects.filter(app=app, node=node).all()[0:49]
    except SpringCloudBackup.DoesNotExist as e:
        return JsonResponse({'msg': str(e)})
