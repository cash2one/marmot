...
class DownloadMixin(object):
    ...
    def download_file(self, url):
        filename = os.path.join(ICE_PROJECT_DIR, os.path.basename(url))
...
class TaskIceDeploy(DownloadMixin, TaskBase):
    ...
    def download_jar(self):
        return self.download_file(self.jar)

    def download_conf(self):
        return self.download_file(self.conf)
    ...
    def deploy(self, jar, conf):
        ...
        shutil.move(jar, os.path.join(self._pkg_dest_path, 'lib'))
        unzip(conf, os.path.join(self._pkg_dest_path, 'config'))
        self.log('处理完成...')
        self.log('>>>>>>>>>>>>>>>>完成<<<<<<<<<<<<<<<<<<')
