...
from .forms import TaskIceServiceForm
...
class TaskFirewallCreate(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        Task.objects.create(name=data['taskName'], applicant=request.user, note=data['taskNote'],
                            content_object=task_firewall)
...
class TaskIceServiceCreate(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        Task.objects.create(name=cleaned_data['name'], applicant=self.request.user, type='ice', deploy=True,
...
def task_implement_log(request, identifier):
    ret = [RDS.lpop(identifier) for _ in xrange(RDS.llen(identifier))]
