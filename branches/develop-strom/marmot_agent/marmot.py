...
MARMOT_HOST = '192.168.56.105'
MARMOT_PORT = 8000

ALLOW_CLIENT_HOSTS = (
    MARMOT_HOST, 'localhost', '127.0.0.1',
    '10.20.0.202', '172.16.20.48','192.168.56.105'
)
...
class DownloadMixin(object):
    ...
    def download_file(self, url, to_dir, fname=None):
        ...
        try:
            logger.info('urllib.urlretrieve(%s,%s)...' % (url,filename))
...
class NodeService(object):
    ...
    def check_storm_jar(self,jar_path,jar_md5):
        logger.info('Received check_storm_jar task: check storm jar - %s %s' % (jar_path,jar_md5))
        if not os.path.isfile(jar_path):
            logger.info('文件不存在!' % str(os.path.basename(jar)))
            return False
        if hashlib.md5(open(jar_path, 'rb').read()).hexdigest() == jar_md5:
            return True
        return False
